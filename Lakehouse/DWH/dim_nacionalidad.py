...
# Función para insertar o actualizar registros en `dim_nacionalidad`
def upsert_dim_nacionalidad(partition):
    ...
    try:
        ...
        query = """
        INSERT INTO dim_nacionalidad (
            id, nombre, name, nombre_nacionalidad, iso2, iso3
        )
        VALUES %s
        ...
        values = [(
            row["id"], row["nombre"], row["name"], row["nombre_nacionalidad"],
            row["iso2"], row["iso3"]
...
# Leer datos desde la tabla `gold_lakehouse.dim_nacionalidad` en Databricks
source_table = (spark.table("gold_lakehouse.dim_nacionalidad")
                .select("id", "nombre", "name", "nombre_nacionalidad", "iso2", "iso3"))

# Aplicar la función a las particiones de datos
try:
    source_table.foreachPartition(upsert_dim_nacionalidad)
    logger.info("Proceso completado con éxito (Upsert en dim_nacionalidad de PostgreSQL).")
except Exception as e:
