...
clientify_df_final = clientify_df_final \
    ...
    .withColumn("sourcesystem", lit("Clientify CESIF")) \
...
#14/01/2025 - sourcesystem se ha cambiado de Clientify a Clientify CESIF
...
for t in clientify_df_final.dtypes:
    if t[1] == 'string':
        clientify_df_final = clientify_df_final.withColumn(t[0], coalesce(col(t[0]), lit('')))
    elif t[1] == 'double':
        clientify_df_final = clientify_df_final.withColumn(t[0], coalesce(col(t[0]), lit(0)))
...
# MAGIC %sql
# MAGIC MERGE INTO silver_lakehouse.clientifydeals
# MAGIC USING clientifydeals_source_view 
# MAGIC ON silver_lakehouse.clientifydeals.id = clientifydeals_source_view.id
